...
export function closeMermaidFullscreen() {
  document.querySelectorAll(".mermaid-fs-overlay").forEach((el) => {
...
export function openMermaidFullscreen(block) {
  ...
  `;
  const viewport = overlay.querySelector(".mermaid-fs-viewport");
  const fsStage = overlay.querySelector(".mermaid-fs-stage");
  const zoomLabel = overlay.querySelector("[data-fs-zoom-label]");
  const srcSvg = stage?.querySelector("svg");
  let svgEl = null;
  let baseW = 400;
  let baseH = 300;

  if (srcSvg) {
    svgEl = srcSvg.cloneNode(true);
  ...
  function onToolbarClick(ev) {
    if (ev.target?.closest?.("[data-fs-close]")) {
      closeMermaidFullscreen();
      return;
    }
    const zbtn = ev.target?.closest?.("[data-fs-zoom]");
    if (!zbtn) return;
  ...
  function onPointerDown(ev) {
    ...
    if (ev.target?.closest?.(".mermaid-fs-toolbar")) return;
    state.dragging = true;
