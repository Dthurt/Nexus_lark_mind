...
export function drawioMarkdownHtml(text) {
...
function setStatus(block, text, isError = false) {
  let el = block.querySelector(".drawio-status-inline");
  if (!el) return;
  if (!text) {
    el.hidden = true;
    el.textContent = "";
...
function ensureChrome(block, xml) {
  ...
  const sourcePre = block.querySelector("pre.drawio-source");
  if (sourcePre) {
    sourcePre.textContent = xml;
    sourcePre.hidden = true;
    sourcePre.classList.remove("is-visible");
  }
  return block.querySelector(".drawio-frame");
}

function applyCollapsed(block, collapsed) {
  block.dataset.collapsed = collapsed ? "1" : "0";
  block.classList.toggle("is-collapsed", !!collapsed);
  const vp = block.querySelector(".drawio-viewport");
  const src = block.querySelector("pre.drawio-source");
  if (collapsed) {
...
function mountFrame(block, iframe, xml) {
...
function closeDrawioFullscreen() {
  document.querySelectorAll(".drawio-fs-overlay").forEach((el) => {
...
function openDrawioFullscreen(block) {
  ...
  overlay.addEventListener("click", (ev) => {
    if (ev.target === overlay || ev.target?.closest?.("[data-fs-close]")) closeDrawioFullscreen();
  });
  document.addEventListener("keydown", onKey);
  overlay._nlmFsCleanup = () => document.removeEventListener("keydown", onKey);
...
function bindControls(root) {
  ...
  root.addEventListener("click", async (ev) => {
    const btn = ev.target?.closest?.("[data-drawio-action]");
    if (!btn || !root.contains(btn)) return;
    const block = btn.closest(".drawio-block");
    if (!block) return;
...
export async function renderDrawioIn(root, { streaming = false } = {}) {
  if (!root) return;
  bindControls(root);
  if (streaming) return;

  const blocks = [...root.querySelectorAll(".drawio-block")];
  for (const block of blocks) {
    if (block.getAttribute("data-processed") === "ok" && block.querySelector("iframe.drawio-frame")?.src) {
      continue;
    }
