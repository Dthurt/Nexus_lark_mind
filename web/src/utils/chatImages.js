...
let lightboxEl = null;
let lightboxState = { items: [], index: 0, scale: 1 };
...
function setLightboxZoom(scale) {
  ...
  const img = lightboxEl?.querySelector?.(".chat-image-lb-img");
...
function openLightbox(items, startIndex = 0) {
  ...
  overlay.addEventListener("click", (ev) => {
    ...
    const btn = ev.target.closest?.("[data-lb-action],[data-lb-nav],[data-lb-zoom]");
    if (!btn) return;
  ...
  stage?.addEventListener(
    ...
    (ev) => {
      ev.preventDefault();
      setLightboxZoom(lightboxState.scale + (ev.deltaY < 0 ? 0.12 : -0.12));
    },
    { passive: false }
  );
...
function collectGalleryItems(gallery) {
  return [...gallery.querySelectorAll(".chat-image")].map((fig) => {
...
function groupConsecutive(root) {
...
export function enhanceChatImages(root) {
  if (!root) return;
