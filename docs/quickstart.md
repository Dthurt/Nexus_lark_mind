...
顶栏切换 **对话 / 知识库 / 轨迹**，或直接打开 http://127.0.0.1:8000/knowledge ：知识库是独立页面（检索、写入），针对该库提问回到主对话（输入框可选择知识库）。**添加文件**会把文档写入本会话知识库（其他会话不可见）。详见 [knowledge-base.md](./knowledge-base.md)。
